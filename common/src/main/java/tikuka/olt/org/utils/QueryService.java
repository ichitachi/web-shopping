package tikuka.olt.org.utils;

import org.springframework.data.jpa.domain.Specification;
import tikuka.olt.org.common.Filter;
import tikuka.olt.org.common.RangeFilter;
...
public abstract class QueryService<ENTITY> {
        ...
        if (filter.getEquals() != null) {
            ...
            if(filter.getLikeType() != null) {
                if(filter.getLikeType().equalsIgnoreCase("left")) {
                    return leftLikeUpperSpecification(field, filter.getContains());
                }else if(filter.getLikeType().equalsIgnoreCase("right")) {
                    return rightLikeUpperSpecification(field, filter.getContains());
                }
            }
    ...
    protected <OTHER> Specification<ENTITY> likeUpperReferringEntitySetSpecification(SingularAttribute<? super ENTITY, OTHER> reference,
                                                                      SingularAttribute<OTHER, String> idField,
                                                                      String value) {
        return (root, query, builder) -> builder.like(builder.upper(root.join(reference).get(idField)), wrapLikeQuery(value));
    }
